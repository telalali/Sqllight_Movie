...
def make_movie(row):
    return Movie(row["movieID"], row["name"], row["year"], row["minutes"], row["desc"],
            row["director"], row["rating"], row["cover_image"], make_category(row))
...
def get_movies_by_category(category_id):
    query = '''SELECT movieID, Movie.name, year, minutes, desc, director,
                          rating, cover_image, Movie.categoryID as categoryID,
...
def get_movies_by_year(year):
     query = '''SELECT movieID, Movie.name, year, minutes, Movie.desc, director,
                          rating, cover_image
...
def add_movie(movie):
    sql = '''INSERT INTO Movie (categoryID, name, year,  minutes, desc, director,
                          rating, cover_image) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)'''
    with closing(conn.cursor()) as c:
        c.execute(sql, (movie.category.id, movie.name, movie.year, movie.minutes,
                        movie.desc, movie.director, movie.rating, movie.cover_image))
        conn.commit()
