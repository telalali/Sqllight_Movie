...
def display_movies(movies, title_term):
    ...
    line_format = "{:3s} {:25s} {:6s} {:5s} {:13s} {:8s} {:7s} {:7s} {:10s}"
    print(line_format.format("ID", "Name", "Year", "Mins", "Desc", "Director",
                          "Rating", "Cover_image", "Category"))
    print("-" * 95)
    for movie in movies:
        print(line_format.format(str(movie.id), movie.name,
                                 str(movie.year), str(movie.minutes), str(movie.desc),
                                 str(movie.director), str(movie.rating), str(movie.cover_image),
                                 movie.category.name))
    print()    
...
def add_movie():
    ...
    if category == None:
        ...
        movie = Movie(name=name, year=year, desc=desc, cover_image=cover_image,
                     director=director, rating=rating, minutes=minutes,
                      category=category)
        db.add_movie(movie)    
